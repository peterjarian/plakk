...
When working inside `/Users/peter/.codex/worktrees/**`, completing a code change means:

1. Run the required checks.
2. Commit only the scoped change.
3. Push the branch.
4. Open a draft PR.
5. Report the PR URL and any unrelated dirty files.

Do not wait for a separate "push/open PR" prompt unless the user explicitly says not to commit or not to push.

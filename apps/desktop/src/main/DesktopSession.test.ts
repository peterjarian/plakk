...
import { DesktopProjection, type DesktopProjectionUpdate } from "./DesktopProjection.ts";
...
describe("DesktopSession", () => {
  it("cannot restore stale refresh or callback work after explicit sign-out starts", async () => {
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const projectionUpdates: Array<DesktopProjectionUpdate> = [];
          ...
          Layer.succeed(
            ...
            DesktopAccountData.of({
              purge: () =>
                Effect.sync(() => {
                  accountPurged = true;
                  projectionUpdates.push({ kind: "signed-out" });
                }),
            }),
          ),
          Layer.succeed(
            ...
            DesktopProjection.of({
              ...
              refresh: Effect.void,
              update: (update) => Effect.sync(() => void projectionUpdates.push(update)),
            }),
          ),
          NativeFileSources.Live,
          Layer.succeed(
            SnippetUploadEngine,
        ...
        return yield* Effect.gen(function* () {
          ...
          const refreshProjectionUpdates = [...projectionUpdates];
          ...
          return {
            ...
            credentialsPurged,
            projectionUpdates,
            refreshProjectionUpdates,
          };
        }).pipe(Effect.provide(DesktopSession.Live.pipe(Layer.provide(dependencies))));
      }),
    );
    ...
    expect(result.refreshProjectionUpdates).not.toContainEqual({ kind: "offline", account });
    expect(result.credentialEvents.at(-1)).toBe("credentials-purged");
    expect(result.projectionUpdates.at(-1)).toEqual({ kind: "signed-out" });
  });
});

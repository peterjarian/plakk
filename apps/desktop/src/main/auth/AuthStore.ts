...
class AuthStoreError extends Schema.TaggedErrorClass<AuthStoreError>()("AuthStoreError", {
  cause: Schema.Defect(),
...
export const AuthStore: Effect.Effect<AuthStoreShape, AuthStoreError> = Effect.gen(function* () {
  ...
  const writeEncryptedString = Effect.fn("AuthStore.writeEncryptedString")(function* (
    ...
    yield* Effect.try({
      try: () => {
        store.set(key, value === null ? null : safeStorage.encryptString(value).toString("base64"));
      },
  ...
  const set = Effect.fn("AuthStore.set")(function* <Key extends AuthStoreKey>(
    ...
    yield* writeEncryptedString(key, value === null ? null : yield* encodeStoredValue(key, value));
  });

...
import { useSnippetState } from "../state/snippets.ts";
...
export function Home() {
  ...
  const snippets = useSnippetState({ accountBlocked });
    ...
    () =>
      window.ipc.clipboard.onPaste((content) => {
        ...
        if (content.type === "text") {
          snippets.addText(content.text);
          return;
        }

        if (content.type === "image") {
          snippets.setIssue("Pasted images need file upload support. Drag the file in for now.");
          return;
        }

        if (content.type === "file") {
          snippets.setIssue(
            "Clipboard file metadata is not enough to upload yet. Drag the file in.",
          );
        }
      }),
    [accountBlocked, snippets],
  );
  ...
  return (
    <main
      ...
      onDrop={(event) => {
        ...
        setIsDragging(false);
        if (accountBlocked) return;
        snippets.addDropped(event.dataTransfer);
      }}
    >
      ...
      <div className="scrollbar-hidden min-h-0 flex-1 overflow-y-auto px-6 pb-4">
        <div className="sticky top-0 z-20 bg-background pt-3 pb-5">
          {(accountBlocked || snippets.issue !== null) && (
            <div className="mb-2 flex items-center gap-2 rounded-md bg-muted px-2.5 py-1.5 text-xs text-muted-foreground">
              ...
              <span className="min-w-0 flex-1 truncate">
                {snippets.issue ?? "Sync paused. Finish billing and setup storage to add snippets."}
              </span>
              <Button
                type="button"
                variant="ghost"
          ...
          <SnippetComposer
            ...
            onSubmit={snippets.addText}
            onFiles={snippets.addFiles}
          />
        </div>

        <SnippetList empty={snippets.visibleSnippets.length === 0}>
          {snippets.visibleSnippets.map((snippet) => (
            <SnippetRow
              key={snippet.id}
              snippet={snippet}
              copied={copiedId === snippet.id}
              onCopy={() => copy(snippet)}
              onDelete={() => snippets.deleteSnippet(snippet.id)}
              {...(snippet.kind === "LINK" ? { onOpenLink: openLink } : {})}
              onStopUpload={() => snippets.stopUpload(snippet.id)}
            />
          ))}
        </SnippetList>
      </div>
